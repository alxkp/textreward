...
def openAICall(client, prev_rewards, most_recent_obs, task_desc, obs):
    most_recent_obs = most_recent_obs[35:]
...
def train():
    ...
    client = OpenAI(api_key="sk-proj-yv1vz4D43V0J04NVnprjT3BlbkFJOefGGelxz8niyUlZnlVK")
    ...
    while(True):
        ...
        for step_no in range(agent.max_nb_steps_per_episode):
            ...
            for i in range(len(dones)):
                scores[i] = openAICall(client,prev_rewards[i],most_recent_observation_strings[i],task_desc_strings[i],obs[i])
